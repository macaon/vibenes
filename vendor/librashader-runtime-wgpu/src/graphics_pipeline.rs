...
impl PipelineLayoutObjects {
    pub fn new(
        ...
        // wgpu 29 tightened bind-group validation: every binding the
        // shader references must appear in the pipeline layout.
        // Slang shaders compile their "push constant" range into a
        // second uniform buffer (WGSL has no push-constant concept),
        // and `filter_pass.rs` already binds it at runtime - but the
        // upstream layout code only declared the UBO. Mirror the UBO
        // entry for the push-constant buffer when reflection reports
        // one with a concrete binding, otherwise wgpu rejects the
        // pipeline at create time.
        if let Some(push_meta) = reflection
            .push_constant
            .as_ref()
            .filter(|push| !push.stage_mask.is_empty())
        {
        ...
        // wgpu 29 bumped `bind_group_layouts` from `&[&BindGroupLayout]` to
        // `&[Option<&BindGroupLayout>]` so callers can leave slots empty.
        // We always populate both slots (main + sampler), so just wrap each
        // ref in `Some`.
        let bind_group_layout_refs: [Option<&wgpu::BindGroupLayout>; 2] =
            [Some(&main_bind_group), Some(&sampler_bind_group)];
