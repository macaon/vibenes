...
//! NES (Famicom) emulator core. All NES-specific subsystems - the 6502
//! CPU, the 2A03 APU, the PPU, mapper variants, FDS support, and the
//! iNES ROM loader - live under this module. The shared host glue
//! (window, audio device, save I/O) sits at the crate root and is
//! reused by every console core.
...
impl From<Region> for crate::core::Region {
    fn from(r: Region) -> Self {
        match r {
            Region::Ntsc => crate::core::Region::Ntsc,
            Region::Pal => crate::core::Region::Pal,
        }
    }
}

impl crate::core::Core for Nes {
    fn step_until_frame(&mut self) -> Result<(), String> {
        Nes::step_until_frame(self)
    }

    fn run_cycles(&mut self, cycles: u64) -> Result<(), String> {
        Nes::run_cycles(self, cycles)
    }
    ...
    fn region(&self) -> crate::core::Region {
        Nes::region(self).into()
    }
